...
#define log_rpack(buf, len) __logpkg(__FILE__, __LINE__, __FUNC__, buf, len, RPACK)
#define log_spack(buf, len) __logpkg(__FILE__, __LINE__, __FUNC__, buf, len, SPACK)
...
void __logpkg(const char *fname, int line, const char *func, const char *buf, int len, int level) NORETURN; /* buf logger */
